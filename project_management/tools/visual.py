...
def approve(which):
    # Утверждать надо то, что на проде СЕЙЧАС. Раньше approve копировал
    # последние лежащие снимки — и после деплоя в эталон попадала картинка
    # предыдущей версии: страж потом честно ругался на «расхождение», которого
    # не было. Та же ловушка, что и с compare 31.07 — снимаем заново.
    if not capture():
        print('   снимок не удался — эталон не трогаю')
        return 2
