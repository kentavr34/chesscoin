...
export const restoreAllAttempts = async (): Promise<void> => {
  ...
  console.log(`[Attempts] ✅ Restored +1 for ${result.count} users (max=${max})`);
};
...
export const startAttemptsCron = (): void => {
  ...
  cron.schedule("0 0,8,16 * * *", async () => {
    console.log("[Attempts] Cron tick — restoring...");
    await restoreAllAttempts().catch((err) =>
      console.error("[Attempts Cron] Error:", err)
    );
  });

  console.log("[Attempts] Cron started: +1 every 8h (00:00, 08:00, 16:00 UTC)");
};
