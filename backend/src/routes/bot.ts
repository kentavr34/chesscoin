...
botRouter.get("/stats/detailed", async (_req: Request, res: Response) => {
  try {
    ...
    console.error("[bot/stats/detailed]", err);
...
botRouter.get("/tasks", async (_req: Request, res: Response) => {
  try {
    ...
    res.json(tasks.map((t) => ({
      ...t,
      winningAmount: t.winningAmount.toString(),
    })));
  } catch (err) {
    console.error("[bot/tasks GET]", err);
...
botRouter.post("/tasks/create", async (req: Request, res: Response) => {
  try {
    ...
    console.error("[bot/tasks/create]", err);
...
botRouter.put("/tasks/:id/toggle", async (req: Request, res: Response) => {
  try {
    ...
    console.error("[bot/tasks toggle]", err);
...
botRouter.delete("/tasks/:id", async (req: Request, res: Response) => {
  try {
    ...
    if (err.code === "P2025") return res.status(404).json({ error: "Задание не найдено" });
    console.error("[bot/tasks delete]", err);
...
botRouter.post("/cleanup/dead", async (_req: Request, res: Response) => {
  try {
    ...
    console.error("[bot/cleanup/dead]", err);
...
botRouter.post("/cleanup/sessions", async (_req: Request, res: Response) => {
  try {
    ...
    console.error("[bot/cleanup/sessions]", err);
...
botRouter.post("/restart", async (_req: Request, res: Response) => {
  ...
  setTimeout(() => {
    console.log("[bot/restart] Graceful shutdown by admin command");
    process.exit(0);
  }, 500);
});
