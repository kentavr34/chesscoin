...
### 2026-05-16 · ProfilePage · слитие Игры+Сохранённые с фильтрами
- Сделано: убрана вкладка `saves`, секция «Сохранённые» (только своё) над «Последние партии». Фильтры: имя героя (input), тип партии (BOT/BATTLE/WAR/TOURNAMENT), период (Сегодня/Неделя/Месяц). Игры сортированы по дате убыв. Коммит `2a418f4`.
- Проверка: `git push` → `docker compose up -d --build frontend` на 185.203.118.96, контейнер Up, `https://chesscoin.app/` → 200.
- Решение: ✅ закрыто.
