...
async def cmd_start(message: Message):
    ...
    # N17: Проверяем через backend — новый пользователь или вернувшийся
    is_new = True
    try:
        async with BackendClient() as client:
            result = await client.check_user_exists(str(user.id))
            is_new = not result.get("exists", False)
    except Exception:
        pass  # Если бэкенд недоступен — показываем полное приветствие

    if is_new:
        # Новый пользователь — полное приветствие с бонусом
        await message.answer(
        ...
        # Вернувшийся — короткое приветствие без бонуса
        returning_text = (
            f"{t(lang, 'welcome_returning').format(name=name)}\n\n"
            f"{t(lang, 'welcome_back_text')}"
        )
        await message.answer(
            returning_text,
            reply_markup=play_keyboard(lang),
...
# ─────────────────────────────────────────
# /play — открыть игру (псевдоним /start)
# ─────────────────────────────────────────
