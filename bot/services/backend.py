...
class BackendClient:
    ...
    async def check_user_exists(self, telegram_id: str) -> dict:
        """Проверить зарегистрирован ли пользователь (для /start нового vs вернувшегося)."""
        try:
            return await self._get("/bot/user-exists", {"telegramId": telegram_id})
        except Exception:
            return {"exists": False}
