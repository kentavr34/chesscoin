                  ...
                  <div key={r.rank} style={{
                    ...
                    {r.bonus > 0 && (
                      <div style={{ textAlign: 'right', flexShrink: 0 }}>
                        <div style={{ fontSize: 10, color: isCurrent ? 'var(--accent, #F5C842)' : unlocked ? '#9B85FF' : 'var(--text-muted, #4A5270)', fontWeight: 600 }}>+{r.bonus.toLocaleString()} ᚙ</div>
                        <div style={{ fontSize: 9, color: 'var(--text-muted, #4A5270)' }}>{r.pct}%</div>
                      </div>
                    )}
            ...
            <div style={{ margin: '0 18px 16px', display: 'flex', flexDirection: 'column', gap: 8 }}>
              {rules.map((r) => (
                <div key={r.ico} style={ruleRow}>
                  ...
                  <div>
                    <div style={{ fontSize: 13, fontWeight: 700, color: 'var(--accent, #F5C842)' }}>{r.title}</div>
                    <div style={{ fontSize: 11, color: 'var(--text-secondary, #8B92A8)', marginTop: 2 }}>{r.sub}</div>
                  </div>
                </div>
              ))}
