  ...
  return (
    <div onClick={onClose} style={{ position: 'fixed', inset: 0, zIndex: "var(--z-modal, 300)", background: 'rgba(0,0,0,0.75)', backdropFilter: 'blur(8px)', WebkitBackdropFilter: 'blur(8px)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24 }}>
      <div onClick={e => e.stopPropagation()} style={{ width: '100%', maxWidth: 320, background: 'linear-gradient(160deg,var(--bg-light, #13161F),var(--bg-dark, #0B0D11))', border: `1px solid ${color}40`, borderRadius: 24, padding: '32px 24px 24px', textAlign: 'center', boxShadow: `0 0 60px ${color}20` }}>
        ...
        <button onClick={onClose} style={{ width: '100%', padding: 12, background: 'var(--color-bg-card, #1C2030)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 14, color: 'var(--color-text-primary, #F0F2F8)', fontSize: 13, fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit' }}>
          {t.gameResult.close}
        </button>
      </div>
    </div>
